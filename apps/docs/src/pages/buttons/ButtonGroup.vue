<template>
  <div>
    ...
    <playground :options="options" :config="config" :code="code" display-value @reset="reset">
      <template #component>
        <component :is="ForgeSelectButton" v-bind="options" v-model="opts" />
      </template>
      <template #value>
        {{ opts }}
      </template>
...
const opts = ref<ForgeSelectButtonOption[]>([
  ...
  { label: "Min", value: "2", severity: "warning", selected: true },
...
const code = computed(() => {
  return `
<template>
 <ForgeSelectButton ${propVals.value.join(" ")} v-model="opts"/>
</template>
