<template>
  <div class="py-4 px-3 d-flex" :data-cy="`file-info-${file.name}`">
    ...
    <div class="ms-auto my-auto d-flex align-items-center gap-1">
      ...
      />
      <Button v-if="uploadStatus === 'Not Uploaded' || uploadStatus === 'Failed' || uploadStatus === 'Duplicate' || uploadStatus === 'Aborted'" link @click="uploadBlob">
        <Icon :icon="uploadStatus === 'Not Uploaded' || uploadStatus === 'Duplicate' ? 'bi:upload' : 'bi:arrow-clockwise'" />
      </Button>
      <Button v-if="uploadStatus === 'Uploading'" link @click="controller.abort()">
